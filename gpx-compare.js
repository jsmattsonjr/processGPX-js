...
function interpolateAltitudePoints(lineString, intervalMeters) {
	...
	for (let i = 1; i < coordinates.length; i++) {
		...
		const segmentDistance = turf.distance(currentPoint, nextPoint, {
			units: "meters",
		});
		...
		while (
			nextInterpolationDistance <= segmentEndDistance &&
			nextInterpolationDistance > segmentStartDistance
		) {
			const ratio =
				(nextInterpolationDistance - segmentStartDistance) / segmentDistance;
			...
			if (coordinates[i - 1].length >= 3 && coordinates[i].length >= 3) {
				interpolatedElevation =
					coordinates[i - 1][2] +
					(coordinates[i][2] - coordinates[i - 1][2]) * ratio;
			}
	...
	if (
		...
		(interpolatedPoints.length === 0 ||
			interpolatedPoints[interpolatedPoints.length - 1].distance <
				totalDistance)
	) {
		interpolatedPoints.push({
...
function compareRouteGeometry(lineString1, lineString2, bufferDistance) {
	...
	const buffer1 = turf.buffer(line1, bufferDistance / 1000, {
		units: "kilometers",
	});
	const buffer2 = turf.buffer(line2, bufferDistance / 1000, {
		units: "kilometers",
	});
...
function compareAltitudes(points1, points2, tolerance) {
	...
	if (points1.length === 0 || points2.length === 0) {
		return {
			altitudesEqual: false,
			differences: [],
			maxDifference: Number.POSITIVE_INFINITY,
			avgDifference: Number.POSITIVE_INFINITY,
		};
	}
	...
	for (let distance = 0; distance <= maxDistance; distance += 100) {
		...
		while (
			...
			points1[p1Index + 1].distance <= distance
		) {
			p1Index++;
		}
		while (
			p2Index < points2.length - 1 &&
			points2[p2Index + 1].distance <= distance
		) {
			p2Index++;
		}
	...
	const maxDifference =
		differences.length > 0
			? Math.max(...differences.map((d) => d.difference))
			: 0;
	const avgDifference =
		differences.length > 0
...
function main() {
	...
	console.log(
		`Routes found: ${lineStrings1.length} in file1, ${lineStrings2.length} in file2`,
	);
	...
	const geometryResult = compareRouteGeometry(
		mainRoute1,
		mainRoute2,
		bufferDistance,
	);
	...
	console.log(
		`Route 1 within Route 2 buffer (${bufferDistance}m): ${geometryResult.route1WithinRoute2Buffer ? "✅" : "❌"}`,
	);
	console.log(
		`Route 2 within Route 1 buffer (${bufferDistance}m): ${geometryResult.route2WithinRoute1Buffer ? "✅" : "❌"}`,
	);
	console.log(
		`Routes geometrically equal: ${geometryResult.routesEqual ? "✅" : "❌"}`,
	);
	console.log("");

	// Compare altitudes
	console.log("📈 Altitude Comparison:");
	const altitudePoints1 = interpolateAltitudePoints(
		mainRoute1,
		altitudeInterval,
	);
	const altitudePoints2 = interpolateAltitudePoints(
		mainRoute2,
		altitudeInterval,
	);

	console.log(
		`Altitude points: ${altitudePoints1.length} from route 1, ${altitudePoints2.length} from route 2`,
	);

	const altitudeResult = compareAltitudes(
		altitudePoints1,
		altitudePoints2,
		altitudeTolerance,
	);
	...
	console.log(
		`Max altitude difference: ${altitudeResult.maxDifference.toFixed(2)}m`,
	);
	console.log(
		`Average altitude difference: ${altitudeResult.avgDifference.toFixed(2)}m`,
	);
	console.log(
		`Altitudes within tolerance (${altitudeTolerance}m): ${altitudeResult.altitudesEqual ? "✅" : "❌"}`,
	);
	console.log("");

	// Overall result
	const routesEqual =
		geometryResult.routesEqual && altitudeResult.altitudesEqual;
	console.log(
		`🏁 Overall Result: Routes are ${routesEqual ? "EQUAL" : "NOT EQUAL"} ${routesEqual ? "✅" : "❌"}`,
	);
